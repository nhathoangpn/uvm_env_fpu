...
@ sva_fin_num = `egrep -c "Error: " run_sim.log`
@ is_hang = `egrep -c "UVM_FATAL.*uvm_phase.*reporter \[PH_TIMEOUT\] Explicit timeout of" run_sim.log`

if ($is_hang == 1) then
	set LINE = "\/\/ sva error number =        NA         \/\/"
else
	set LINE = "\/\/ sva error number =        $sva_fin_num         \/\/"
	if ($sva_fin_num > 0) then
		sed -i "s/SIMULATION PASSED/SIMULATION FAILED/g" run_sim.log
	endif
endif


sed -i "/C   error number/a $LINE" run_sim.log
